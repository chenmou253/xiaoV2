...
翻译执行策略按模型分离：云端 `qwen3.7-flash` 保持整页严格 JSON Schema、单次翻译 + 单次审核、失败不自动重试/不自动切换模型；本地 Qwen3-4B 不再使用 JSON，直接通过 `mlx_lm.generate()` 输出纯文本 S/W 行协议：`S<TAB>segment_index<TAB>translation` 与 `W<TAB>segment_index<TAB>word_index<TAB>meaning<TAB>phonetic`。模型完全不接触真实 segment/word ID，程序按临时索引恢复原 OCR ID，并严格检查所有 S/W 是否齐全、重复或越界。协议错误或输出截断只允许整页重生成 1 次，第二次仍失败即停止并保留原页面；不拆分、不猜修、不自动切换云模型。Reviewer 同样使用独立的 S/W/OK 行协议。本地路径明确绕过 `generate_structured()`，因此不会再附加 JSON Schema 指令。

...
翻译执行策略按模型分离：云端 `qwen3.7-flash` 保持整页严格 JSON Schema、单次翻译 + 单次审核、失败不自动重试/不自动切换模型；本地 Qwen3-4B 使用独立的结构化 Prompt，一次处理整页；模型不接触任何真实 segment/word ID，而是只使用临时的零基 `segment_index / word_index`，输入里同时显式给出 `segment_count / word_count`。模型必须完整返回这些临时索引，程序再据此恢复原 OCR 的真实 ID，因此即使模型调整数组顺序也不会错位。非法 JSON、截断或索引/数组结构/数量不匹配只允许整页重生成 1 次，第二次仍失败即停止并保留原页面；不递归拆分、不猜修 JSON、不自动切换云模型。Reviewer 同样只使用临时索引。

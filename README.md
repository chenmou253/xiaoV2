...
翻译执行策略按模型分离：云端 `qwen3.7-flash` 保持整页严格 JSON Schema、单次翻译 + 单次审核、失败不自动重试/不自动切换模型；本地 Qwen3-4B 使用独立的结构化 Prompt，一次处理整页，但模型不再生成任何 segment/word ID，只返回与输入数组等长、同顺序的 `translation / meaning / phonetic`，程序按原 OCR 顺序恢复真实 ID。非法 JSON、截断或数组结构/数量不匹配只允许整页重生成 1 次，第二次仍失败即停止并保留原页面；不递归拆分、不猜修 JSON、不自动切换云模型。Reviewer 同样使用索引而不是业务 ID。

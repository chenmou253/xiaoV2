...
翻译执行策略按模型分离：云端 `qwen3.7-flash` 保持整页严格 JSON Schema、单次翻译 + 单次审核、失败不自动重试/不自动切换模型；本地 Qwen3-4B 使用常驻 MLX daemon，将页面按最多 4 个 segment / 约 20 个 word 分批处理，非法或截断 JSON 只允许当前小批次重生成 1 次，漏词只补缺失项，前导零 ID 可安全规范化，真实错误 ID 仍直接拒绝。所有本地批次完成并做整页 ID 一致性校验后才写回页面，因此中途失败不会留下半页翻译结果。

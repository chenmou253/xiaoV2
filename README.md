...
翻译执行策略按模型分离：云端 `qwen3.7-flash` 保持整页严格 JSON Schema、单次翻译 + 单次审核、失败不自动重试/不自动切换模型；本地 Qwen3-4B 恢复使用独立 JSON Prompt，一次处理整页。Translator 固定使用 `LOCAL_TRANSLATOR_SYSTEM_PROMPT + local_translation_prompt()`，返回 `segments[].translation` 与 `segments[].words[].meaning/phonetic`，不让模型生成真实 segment/word ID，程序按输入数组顺序恢复 OCR ID；Reviewer 固定使用 `LOCAL_REVIEWER_SYSTEM_PROMPT + local_review_prompt()`。非法 JSON、截断或数组结构/数量不匹配只允许整页重生成 1 次，第二次仍失败即停止；不拆分、不猜修、不自动切换云模型。

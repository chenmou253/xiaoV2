...
export default function DeviceCheck({role,id}:{role:'teacher'|'student';id:number}){
 ...
 const status=(value:CheckStatus)=><span className={`device-status device-status-${value}`}>{statusText[value]}</span>;
 return <main className="device-shell"><a href={role==='teacher'?'/teacher/lessons':'/account/lessons'}>← 我的课程</a><h1>课前设备检测</h1><p>确认摄像头、麦克风和扬声器可用，再进入课堂。</p>{debugEarlyEntry&&<p className="admin-note">本地调试模式：允许提前进入课堂，课程结束后仍不可进入。</p>}{error&&<p className="admin-error">{error}</p>}<div className="device-grid"><div className="device-preview"><video ref={video} autoPlay playsInline muted/>{cameraStatus!=='available'&&<span>{cameraStatus==='checking'?'正在检测摄像头…':'摄像头不可用'}</span>}</div><section><div className="device-control"><label>摄像头<select value={camera} onChange={e=>setCamera(e.target.value)}><option value="">默认摄像头</option>{cameras.map(device=><option key={device.deviceId} value={device.deviceId}>{device.label||'摄像头'}</option>)}</select></label>{status(cameraStatus)}</div><div className="device-control"><label>麦克风<select value={microphone} onChange={e=>setMicrophone(e.target.value)}><option value="">默认麦克风</option>{microphones.map(device=><option key={device.deviceId} value={device.deviceId}>{device.label||'麦克风'}</option>)}</select></label>{status(microphoneStatus)}</div><div className="device-control"><span>扬声器</span><button type="button" onClick={()=>void testSpeaker()}>{speakerStatus==='checking'?'正在测试…':'测试扬声器'}</button>{status(speakerStatus)}</div><p className="device-network">网络：{networkStatus==='available'?<>可用 · {network} {status(networkStatus)}</>:<>{network} {status(networkStatus)}</>}</p><button className="admin-primary" disabled={!ready||!canEnter} onClick={()=>{sessionStorage.setItem(`classroom-devices-${role}-${id}`,JSON.stringify({camera,microphone}));stream.current?.getTracks().forEach(track=>track.stop());location.assign(`${role==='teacher'?'/teacher':''}/classroom/${id}`)}}>{canEnter?'进入课堂':'上课前 10 分钟可进入'}</button></section></div></main>;
}

...
import {booksAPI,type Accent,type Book,type BookPage,type PageContent,type Segment,type Word} from './api';
...
function Header({leave}:{leave:()=>void}){return <header className="topbar"><button className="brand" onClick={leave} aria-label="回到书架"><span className="brand-icon"><BookOpen size={25}/></span><span>小小点读家<small>LITTLE READERS CLUB</small></span></button><nav className="site-links"><a href="/account">账号</a><a href="/admin">后台</a></nav></header>}
...
function Shelf({state,onRetry}:{state:ShelfState;onRetry:()=>void}){
 ...
 {state.kind==='ready'&&state.books.length>0&&<div className="books">{state.books.map(book=><button key={book.book_id} className="book-card" onClick={()=>{location.hash=`read?book=${encodeURIComponent(book.book_id)}`}}><div className="book-art"><span className="book-tab">{book.semester||book.grade||'英语'}</span><div className="cover-text"><small>{[book.publisher,book.grade].filter(Boolean).join(' · ')||'ENGLISH'}</small><strong>{book.title}</strong><span>{book.subtitle}</span></div><div className="textbook-window">{book.cover?<img src={book.cover} alt={book.title}/>:<BookOpen size={64} aria-label="暂无封面"/>}</div><span className="sample-tag"><Sparkles size={14}/>{book.page_count} 页</span></div><div className="book-info"><div><h2>{book.title}</h2><p>{book.publisher||book.description}</p></div><span className="round-arrow"><ArrowRight/></span></div></button>)}</div>}
...
function Reader({book}:{book:Book}){
 ...
 useEffect(()=>{const ctrl=new AbortController();setError('');setPages(null);booksAPI.pages(book.book_id,ctrl.signal).then(setPages).catch(e=>{if(e.name!=='AbortError')setError(e.message)});return()=>ctrl.abort()},[book.book_id,retry]);
 ...
 if(!pages.length)return <div className="reader"><Status icon={<BookOpen/>} title="这本书还没有页面" detail="资源生成并导入后即可开始阅读。"><button onClick={leave}>返回书架</button></Status></div>;
 const segments=content?.segments||[],current=segments[segmentIndex];
 return <div className="reader"><div className="reader-heading"><button className="back" onClick={leave}><ArrowLeft size={18}/> 我的书架</button><span>{book.title}</span><select className="page-select" aria-label="选择课本页" value={index} onChange={e=>move(Number(e.target.value))}>{pages.map((item,i)=><option key={item.page} value={i}>第 {item.page} 页 · {item.title}</option>)}</select></div>
 <div className="lesson-heading"><div><span className="eyebrow">{page?.unit}</span><h1>{page?.title}</h1></div><div className="page-arrows"><span>第 {index+1} 页 / {pages.length} 页</span><button disabled={index===0} aria-label="上一页" onClick={()=>move(index-1)}><ChevronLeft/></button><button disabled={index===pages.length-1} aria-label="下一页" onClick={()=>move(index+1)}><ChevronRight/></button></div></div>
 <div className="reading-grid"><section className="page-panel"><div className="panel-bar"><span><BookOpen size={17}/> 我的课本</span><button onClick={()=>setZoom(!zoom)}>{zoom?<ZoomOut size={19}/>:<ZoomIn size={19}/>} {zoom?'缩小':'放大'}</button></div><div className="page-scroll"><div className={`paper ${zoom?'zoomed':''}`}>{page&&<img src={page.image} alt={`${book.title}第${page.page}页`}/>} {segments.map((segment,i)=><Hotspots key={segment.id} segment={segment} selected={segmentIndex===i} active={audio.active} onSentence={()=>{setSegmentIndex(i);setWord(null);audio.speak(segment.id)}} onWord={item=>{setSegmentIndex(i);setWord(item);audio.speak(item.id)}}/>)}</div></div><p className="paper-hint">点单词查意思 · 点小喇叭听整句 · 小字可放大</p></section>
 <aside>{current?<section className="study-card">
  ...
  {hasWordAudio(current)&&<div className={`word-card ${word?'has-word':''}`}>{word?<><div><small>这个词的意思</small><strong>{word.text}</strong>{word.phonetic&&<em>{word.phonetic}</em>}<span>{word.meaning||'词义尚待校对'}</span></div><button onClick={()=>audio.speak(word.id)}><Volume2/></button></>:<><span className="word-cursor">Aa</span><p>哪个单词有点陌生？<small>点一下上面的单词，意思就在这里。</small></p></>}</div>}
 ...
 </section>:<Status icon={<BookOpen/>} title={content?'本页暂无点读内容':'正在打开这一页...'} detail={content?'继续翻页，探索更多内容。':'正在获取页面数据。'}/>}</aside></div>
 <section className="audio-dock"><div className="dock-title"><Headphones/><span>我的朗读设置<small>按你的节奏听</small></span></div><div className="setting"><span>发音</span>{audio.available.length>1?audio.available.map(value=><button key={value} className={audio.accent===value?'active':''} onClick={()=>audio.setAccent(value)}>{audio.accent===value&&<Check size={13}/>} {value==='en-GB'?'英式':'美式'}</button>):<strong>{audio.available.length===1?(audio.available[0]==='en-GB'?'英式':'美式'):'音频生成中'}</strong>}</div><button className="stop-button" onClick={audio.stop}><Square/></button></section>{audio.error&&<p className="voice-status">{audio.error}</p>}</div>
}
...
function useAudio(bookId:string,page:number,available:Accent[]){
 const choose=useCallback(()=>{const preferred=localStorage.getItem('preferredAccent') as Accent|null;return (preferred&&available.includes(preferred)?preferred:available[0])||'en-US'},[available.join('|')]);
 const [accent,setAccent]=useState<Accent>(choose),[active,setActive]=useState(''),[error,setError]=useState('');const player=useRef<HTMLAudioElement|null>(null);

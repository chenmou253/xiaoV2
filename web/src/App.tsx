...
function pageOptionLabel(book:Book,item:BookPage){const title=item.title===`第 ${item.page} 页`?(item.page===1?'封面':''):item.title;return [`第 ${item.page} 页`,item.unit||book.title,title].filter(Boolean).join(' · ')}
...
function Shelf({state,onRetry}:{state:ShelfState;onRetry:()=>void}){
 ...
 {state.kind==='ready'&&state.books.length>0&&<div className="books dynamic-books">{state.books.map(book=><button key={book.book_id} className="book-card available" onClick={()=>{location.hash=`read?book=${encodeURIComponent(book.book_id)}`}}><div className="book-art"><span className="book-tab">{book.semester||book.grade||'英语'}</span><div className="cover-text"><small>{[book.publisher,book.grade].filter(Boolean).join(' · ')||'ENGLISH'}</small><strong>{book.title}</strong><span>{book.subtitle}</span></div><div className="textbook-window">{book.cover?<img src={book.cover} alt={book.title}/>:<BookOpen size={64} aria-label="暂无封面"/>}</div><span className="sample-tag"><Sparkles size={14}/>{book.page_count} 页</span></div><div className="book-info"><div><h2>{book.title}</h2><p>{book.publisher||book.description}</p></div><span className="round-arrow"><ArrowRight/></span></div></button>)}</div>}
...
function Reader({book}:{book:Book}){
 ...
 return <div className="reader"><div className="reader-heading"><button className="back" onClick={leave}><ArrowLeft size={18}/> 我的书架</button><span>{book.title} <span className="slash">/</span> 第 {page?.page ?? index+1} 页</span><select className="page-select" aria-label="选择课本页" value={index} onChange={e=>move(Number(e.target.value))}>{[...new Set(pages.map(item=>item.unit||book.title))].map(unit=><optgroup key={unit} label={unit}>{pages.map((item,i)=>(item.unit||book.title)===unit&&<option key={item.page} value={i}>{pageOptionLabel(book,item)}</option>)}</optgroup>)}</select><span className="sample-badge">{page?.interactive?'可点读':'课本浏览'}</span></div>
 <div className="lesson-heading"><div><span className="eyebrow">{page?.unit}</span><h1>{page?.title}</h1></div><div className="page-arrows"><span className="page-position" aria-live="polite">第 {index+1} 页 / {pages.length} 页</span><button disabled={index===0} aria-label="上一页" onClick={()=>move(index-1)}><ChevronLeft/></button><button disabled={index===pages.length-1} aria-label="下一页" onClick={()=>move(index+1)}><ChevronRight/></button></div></div>
